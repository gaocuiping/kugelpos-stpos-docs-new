...
if [ "$new_version" != "$SCRIPT_VERSION" ]; then
    # 3. ダウンロード
    download_artifact "$url" "/tmp/startup.sh.new" "$checksum"
...
#### Blob Storage
- 時間制限付きSAS URL（Shared Access Signature）
- 読み取り専用、30分間有効
- IPアドレス制限（オプション）
...
for artifact in $(echo "$MANIFEST" | jq -c '.artifacts[] | select(.action == "update")'); do
  ...
  echo "Downloading updated file: $NAME"
  download_and_verify "$URL" "$CHECKSUM" "$NAME"
done
...
for image in $(echo "$MANIFEST" | jq -c '.container_images[] | select(.action == "update")'); do
  ...
  echo "Pulling updated image: $SERVICE"
  docker pull "$IMAGE"
...
def get_script_info(script_name: str, version: str) -> Dict:
    ...
    # SAS URL生成（30分有効）
    sas_url = generate_sas_url(blob_path, expiry_minutes=30)
