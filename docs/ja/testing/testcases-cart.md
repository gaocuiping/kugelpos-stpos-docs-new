...
| **CT-U-301** | `Cache & Master` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_cache_miss`<br>`test_cache_remove`<br>`test_cache_set_and_get`<br>`test_cache_update`<br>`test_get_channel_cache_stats`<br>`test_get_item_by_code_from_cache`<br>`test_get_item_by_code_multiple_calls_reuse_stub`<br>`test_get_item_by_code_uses_module_level_stub`<br>`test_set_item_master_documents`<br>`test_unregistered_item_error`<br>`test_get_tenant_terminal_ids` | キャッシュ（TTL、Miss時振る舞い）やマスタ参照の各種バリエーションと未登録商品ハンドリング。 |
| **CT-U-302** | `Session/Channel` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_close_all_channels`<br>`test_close_session`<br>`test_close_without_channels_is_safe`<br>`test_close_without_session_is_safe`<br>`test_get_session_after_close_creates_new_session`<br>`test_get_session_creates_new_session`<br>`test_get_stub_after_close_creates_new_channel`<br>`test_get_stub_creates_channel_on_first_call`<br>`test_get_stub_reuses_existing_channel`<br>`test_multiple_close_calls_are_safe`<br>`test_session_connector_configuration` | Dapr や gRPC セッション・チャネルの多重・不正クローズに対する安全な回復処理とコネクション再利用。 |
| **CT-U-303** | `Payment & Status` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_bill_with_insufficient_balance`<br>`test_get_status_by_transaction_not_found`<br>`test_get_status_for_transactions`<br>`test_get_transaction_list_with_status_empty_list`<br>`test_line_item_operations`<br>`test_transaction_operations`<br>`test_transaction_status_in_list`<br>`test_transaction_status_in_single_get`<br>`test_payment_process`<br>`test_stamp_duty`<br>`test_update_sales_info_async_empty_cart` | 残高不足決済、空カートでの計算、印紙税適用しきい値、トランザクション状態の空配列等、取引異常系。 |
| **CT-U-304** | `Void, Return & State` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_duplicate_return_prevention`<br>`test_mark_as_refunded_new_document`<br>`test_mark_as_voided_existing_document`<br>`test_reset_refund_status_no_existing_document`<br>`test_reset_refund_status_preserves_void_info`<br>`test_reset_refund_status_success`<br>`test_resume_item_entry_clears_all_payments`<br>`test_resume_item_entry_preserves_line_items`<br>`test_return_async_checks_refund_status`<br>`test_return_async_checks_void_status`<br>`test_return_transaction_success`<br>`test_void_async_checks_status_history`<br>`test_void_returned_transaction_prevention` | レジューム時の支払情報リセット、取消/返品の二重実行抑止、ステータス履歴の参照等、状態遷移の防御機能。 |
| **CT-U-305** | `Text & UI` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_fixed_left_japanese_truncate`<br>`test_fixed_left_without_truncate`<br>`test_line_split_simulation`<br>`test_truncate_text_ascii`<br>`test_truncate_text_mixed`<br>`test_truncate_text_with_suffix` | レシート・ジャーナル印字用のテキストフォーマット（サフィックス付き全角/半角混在時のパディング強制改行等）。 |
