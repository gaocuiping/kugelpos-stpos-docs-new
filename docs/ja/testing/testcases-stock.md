...
| **SK-U-301** | `Cron Triggers` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_create_cron_trigger_daily`<br>`test_create_cron_trigger_invalid_interval`<br>`test_create_cron_trigger_monthly`<br>`test_create_cron_trigger_weekly` | 自動ストックスナップショット向けの各種周期（Cron）パターンのバリデーション検証。 |
| **SK-U-302** | `Schedule Operations` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_delete_snapshot_schedule`<br>`test_get_snapshot_schedule_after_delete`<br>`test_get_snapshot_schedule_after_update`<br>`test_get_snapshot_schedule_default`<br>`test_update_snapshot_schedule_retention_validation`<br>`test_update_snapshot_schedule_success`<br>`test_update_snapshot_schedule_validation_error` | スケジュール設定自体のCRUD、保存期間チェックなどのスケジューラー管理機能。 |
| **SK-U-303** | `Tenant Parameters` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_remove_tenant_schedule`<br>`test_set_reorder_parameters`<br>`test_update_tenant_schedule_disabled`<br>`test_update_tenant_schedule_enabled` | 店舗（テナント）ごとの発注点設定の有効無効化、固有スケジュールの設定操作検証。 |
| **SK-U-304** | `Snapshot Generate` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_create_snapshot`<br>`test_create_snapshot_with_generate_date_time`<br>`test_stock_snapshot_includes_reorder_fields` | 特定日時でのスナップショットデータ生成ロジックの正確性と、発注点情報包含の検証。 |
| **SK-U-305** | `Snapshot Retrieve` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_get_snapshots_by_date_range`<br>`test_get_snapshots_with_end_date_only`<br>`test_get_snapshots_with_start_date_only`<br>`test_get_snapshots_without_date_range` | スナップショット検索時の期間（日付）指定の組み合わせ・境界条件の正常系検証。 |
| **SK-U-306** | `History & Pagination` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_pagination_parameters`<br>`test_stock_history` | 履歴系巨大データのページネーション（Limit/Offset等）によるクエリ安定性。 |
| **SK-U-307** | `Minimum Stock Alert` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_get_reorder_alerts_empty`<br>`test_minimum_stock_alert`<br>`test_update_stock_triggers_reorder_alert`<br>`test_websocket_minimum_stock_alert`<br>`test_websocket_no_alert_when_above_thresholds` | 閾値を割った際の在庫不足アラート発生と、満たしている際の非発火境界の検証。 |
| **SK-U-308** | `Reorder & WS Alert` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_websocket_connection`<br>`test_websocket_reorder_alert`<br>`test_websocket_reorder_alert_new_item` | 発注点レベルに対する警告イベントと、新規商品登録時の WebSocket ブロードキャストの疎通。 |
| **SK-U-309** | `Store Scope Queries` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_get_all_tenant_ids`<br>`test_get_target_stores_specific`<br>`test_get_stock` | 複数テナント混在環境でのストアID指定横断クエリ等による情報漏洩防止と正当性。 |
| **SK-U-310** | `Lifecycle & Health` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_health_check`<br>`test_get_status`<br>`test_shutdown`<br>`test_cleanup_test_data` | 起動中・シャットダウン時のステータスハンドリングやテストデータ自己修復とクリーンアップ。 |
