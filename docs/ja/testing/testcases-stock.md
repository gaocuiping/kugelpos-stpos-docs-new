...
| **SK-U-301** | `Snapshot Scheduler` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_create_cron_trigger_daily`<br>`test_create_cron_trigger_invalid_interval`<br>`test_create_cron_trigger_monthly`<br>`test_create_cron_trigger_weekly`<br>`test_delete_snapshot_schedule`<br>`test_get_snapshot_schedule_after_delete`<br>`test_get_snapshot_schedule_after_update`<br>`test_get_snapshot_schedule_default`<br>`test_remove_tenant_schedule`<br>`test_set_reorder_parameters`<br>`test_update_snapshot_schedule_retention_validation`<br>`test_update_snapshot_schedule_success`<br>`test_update_snapshot_schedule_validation_error`<br>`test_update_tenant_schedule_disabled`<br>`test_update_tenant_schedule_enabled` | 在庫スナップショットのクーロン表現、保存期間バリデーション、無効化といった自動スケジューリング全般。 |
| **SK-U-302** | `Snapshot Retrieve` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_create_snapshot`<br>`test_create_snapshot_with_generate_date_time`<br>`test_get_snapshots_by_date_range`<br>`test_get_snapshots_with_end_date_only`<br>`test_get_snapshots_with_start_date_only`<br>`test_get_snapshots_without_date_range`<br>`test_pagination_parameters`<br>`test_stock_history`<br>`test_stock_snapshot_includes_reorder_fields` | 特定日時でのスナップショット生成、および期間・ページネーション指定による在庫履歴照会の各種境界。 |
| **SK-U-303** | `WS & Alerts` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_get_reorder_alerts_empty`<br>`test_minimum_stock_alert`<br>`test_update_stock_triggers_reorder_alert`<br>`test_websocket_connection`<br>`test_websocket_minimum_stock_alert`<br>`test_websocket_no_alert_when_above_thresholds`<br>`test_websocket_reorder_alert`<br>`test_websocket_reorder_alert_new_item` | 発注点アラート（ゼロ件、閾値異常）および WebSocket 経由でのリアルタイム在庫警告イベントの配信。 |
| **SK-U-304** | `Env & Queries` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_cleanup_test_data`<br>`test_get_all_tenant_ids`<br>`test_get_status`<br>`test_get_stock`<br>`test_get_target_stores_specific`<br>`test_health_check`<br>`test_shutdown` | テストデータクリーンアップ、テナント横断の照会、アプリーケーションの起動・シャットダウンやヘルスチェック全般。 |
