...
| **RP-A-HAN** | **Handle transaction logs received via Dapr pub/sub.** | `API / POST` | ![Missing](https://img.shields.io/badge/Status-Missing-red) | `handle_tranlog` | 系统自动追加的代码接口测试 |
...
| **RP-U-303** | **複雑な分割払いと端数検証** | `Complex Payment Splits` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_extreme_split_payments_with_multiple_taxes` (複数税率での極端な分割払)<br>`test_mixed_transaction_types_with_cartesian_risk` (混合取引のデカルト積リスク防御)<br>`test_multi_transaction_payment_integrity` (複数取引の支払整合性)<br>`test_multiple_payment_methods_mixed` (複数決済手段の混在)<br>`test_rounding_one_yen_difference` (1円の端数丸め差異) | 複数税率での極端な分割払いや端数丸めによる「1円のズレ」許容検証、複数決済手段混在時のデカルト積リスク防御機能。 |
| **RP-U-304** | **返品と返金処理の検証** | `Return Transactions` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_return_exceeds_sales` (売上超過の返品)<br>`test_return_transaction_basic` (基本的な返品取引)<br>`test_return_transactions_aggregation` (返品取引の集計)<br>`test_with_returns` (返品を含む集計) | 返品（返金）に関する基本ロジックや、売上額を超過する返金といったエラー・境界値の検証。 |
| **RP-U-305** | **取消操作の振る舞い検証** | `Void Transactions` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_multiple_voids_same_transaction_type` (同一取引の複数回取消)<br>`test_void_return_basic` (基本的な取消返品)<br>`test_void_sales_basic` (基本的な取消売上) | 取消（Void）操作の基本動作と、同一取引に対する複数回の取消試行等の防御的振る舞いの検証。 |
| **RP-U-306** | **割引とフラグ制御の検証** | `Discounts & Flags` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_cancelled_flag_exclusion` (キャンセルフラグの除外)<br>`test_with_discounts` (割引適用時の集計) | 赤黒処理等のためのキャンセル・フラグ指定の除外要件、および割引適用時の集計の正確性。 |
| **RP-U-307** | **カテゴリ・商品別レポート検証** | `Category & Item Reports` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_category_report_format_verification` (カテゴリ報告書式)<br>`test_category_report_operations` (カテゴリ報告挙動)<br>`test_category_report_with_date_range` (カテゴリ報告・期間指定)<br>`test_item_report_operations` (商品報告挙動)<br>`test_item_report_with_date_range` (商品報告・期間指定) | カテゴリ別、商品別のレポート生成時のフォーマット検証、および期間指定時の正確なデータ集約。 |
| **RP-U-308** | **決済手段別レポート検証** | `Payment Reports` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_payment_report_basic` (基本の支払報告)<br>`test_payment_report_date_validation` (支払報告・日付検証)<br>`test_payment_report_error_handling` (支払報告・エラー処理)<br>`test_payment_report_via_api` (API経由の支払報告)<br>`test_payment_report_with_date_range` (支払報告・期間指定) | 決済手段別レポート特有のバリデーション・エラー処理やAPI呼び出し時の振る舞い検証。 |
| **RP-U-309** | **日次・速報レポート汎用検証** | `General Scope Reports` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_daily_report_accepts_date_range` (日次報告・期間指定)<br>`test_flash_report_rejects_date_range_terminal` (速報・範囲拒否・端末)<br>`test_report_operations` (報告生成挙動)<br>`test_store_report_without_requesting_terminal` (店舗報告・端末未指定) | 日次レポート、フラッシュレポート等の期間（Range）指定の可否判断や、特定端末非指定（店舗全体）時の集約検証。 |
| **RP-U-310** | **複数店舗・端末の集計検証** | `Multi-Store & Terminal` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_multi_store_mixed_transactions` (複数店舗の混合取引)<br>`test_multi_store_multi_terminal_uniqueness` (複数店舗・端末の一意性) | 複数店舗および複数端末が混在する中でのトランザクション集計時の一意性と他テナント漏洩防御。 |
| **RP-U-311** | **ジャーナルパブリッシュの検証** | `Journal Messaging` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_send_daily_report_to_journal` (ジャーナルへの日次送信)<br>`test_send_flash_report_to_journal` (ジャーナルへの速報送信)<br>`test_unknown_report_scope_skips_journal` (未知スコープの送信スキップ) | 精算完了時のジャーナル送信機能、および未定義スコープのジャーナルパブリッシュのスキップ検証。 |
| **RP-U-312** | **基盤・ヘルスチェックの検証** | `Health & Base Setup` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_flush_backward_compatibility` (フラッシュ後方互換性)<br>`test_health_endpoint` (ヘルスチェック経由)<br>`test_health_endpoint_without_dapr` (Dapr無しのヘルスチェック)<br>`test_api_key_request_creates_journal` (APIキー要求でのジャーナル生成) | Dapr 未接続時のヘルスチェック縮退動作や、API Key認証、DB後方互換性等インフラ基盤の検証。 |
...
| **RP-I-003** | **Error Handlersの検証** | `Error Handlers` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_journal_error_does_not_fail_report` / `test_payment_report_empty_data` | 一部のジャーナルデータが破損・欠落していてもレポート出力全体がクラッシュせず、取得可能分を正常にフォールバック返却すること。 |
