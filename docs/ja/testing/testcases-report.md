...
| **RP-U-301** | `Tax & Payment` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_mixed_transaction_types_with_cartesian_risk`<br>`test_multi_transaction_payment_integrity`<br>`test_multiple_payment_methods_mixed`<br>`test_payment_amount_calculation`<br>`test_payment_sum_equals_sales_with_tax`<br>`test_rounding_one_yen_difference`<br>`test_sales_report_amount_bug`<br>`test_tax_breakdown_label`<br>`test_empty_taxes_array`<br>`test_external_tax_only`<br>`test_internal_tax_only`<br>`test_zero_amount_transaction`<br>`test_zero_taxes`<br>`test_extreme_split_payments_with_multiple_taxes` | 税や決済のバリエーション（ゼロ円、複数税、複雑な分割払い、端数処理など）での金額整合性を検証。 |
| **RP-U-302** | `Cancellation & Return` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_cancelled_flag_exclusion`<br>`test_multiple_voids_same_transaction_type`<br>`test_return_exceeds_sales`<br>`test_return_transaction_basic`<br>`test_return_transactions_aggregation`<br>`test_void_return_basic`<br>`test_void_sales_basic`<br>`test_with_discounts`<br>`test_with_returns` | キャンセルや返品の挙動（同一取引の複数回取消、売上超過返品、フラグ除外など）がレポートに正しく反映・控除されること。 |
| **RP-U-303** | `Report Scopes & Formats` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_category_report_format_verification`<br>`test_category_report_operations`<br>`test_category_report_with_date_range`<br>`test_daily_report_accepts_date_range`<br>`test_flash_report_rejects_date_range_terminal`<br>`test_item_report_operations`<br>`test_item_report_with_date_range`<br>`test_payment_report_basic`<br>`test_payment_report_date_validation`<br>`test_payment_report_error_handling`<br>`test_payment_report_via_api`<br>`test_payment_report_with_date_range`<br>`test_report_operations`<br>`test_store_report_without_requesting_terminal` | 各種レポート（カテゴリ、商品、支払等）の期間指定フォーマットエラーや境界値チェック、バリデーション。 |
| **RP-U-304** | `Integrity & Base` | ![Implemented](https://img.shields.io/badge/Status-Implemented-green) | `test_flush_backward_compatibility`<br>`test_health_endpoint`<br>`test_health_endpoint_without_dapr`<br>`test_multi_store_mixed_transactions`<br>`test_multi_store_multi_terminal_uniqueness`<br>`test_send_daily_report_to_journal`<br>`test_send_flash_report_to_journal`<br>`test_unknown_report_scope_skips_journal`<br>`test_api_key_request_creates_journal` | 店舗・端末を跨ぐ一意性、後方互換性、ジャーナル転送、ヘルスチェックおよび特殊権限APIアクセス記録などインフラ層検証。 |
