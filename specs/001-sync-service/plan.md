...
| ターミナル状態変更取得 | Sync → terminal | **Dapr Service Invocation** | `GET /api/v1/sync/status?edge_id={edge_id}` | 特定エッジの状態変更取得 |
...
| ファイル保管 | Sync → Cloud Storage | **Direct API** | S3/GCS/Azure Blob | 大容量ファイル、長期保管、コスト効率 |
...
| ターミナル状態取得 | Sync → terminal | **Dapr Service Invocation** | `GET /api/v1/sync/status` | 定期的なポーリング |
