...
- **ローカルキュー容量**: 10,000レコード**または**100MB（いずれか先に到達した閾値）。エッジ→クラウド送信時、ローカルキュー容量超過時は送信済み（`sync_status='sent'`）データから削除され、未送信データ（`pending`）は保持される。環境変数: `TRANSACTION_QUEUE_MAX_RECORDS`（デフォルト: 10000）、`TRANSACTION_QUEUE_MAX_SIZE_MB`（デフォルト: 100）

**Scale/Scope**:
- **エッジ端末数**: 最大1,000台（同時接続想定）
- **データ種別**: 3種類（master_data: マスターデータ、transaction_log: トランザクションログ（journal_text含む）、terminal_state: ターミナル状態変更）
...
| トランザクションログ配信 | Sync → cart, terminal, journal | **Dapr Pub/Sub (Push)** | トピック: `tranlog_report`, `cashlog_report`, `opencloselog_report` | 既存トピック活用、複数サービスへの配信（journal_textフィールド経由でJournalサービスへも自動配信） |
...
**log_type → Pub/Subトピック対応表**:

| log_type (TransactionLog) | 配信先トピック | 受信サービス | 備考 |
|---------------------------|--------------|-------------|------|
| `transaction` | `tranlog_report` | Cart, Terminal, Journal, Report | 売上取引、返品、値引き等（journal_text含む） |
| `opening_closing` | `opencloselog_report` | Terminal, Journal, Report | レジ開設・精算（journal_text含む） |
| `cash_inout` | `cashlog_report` | Terminal, Journal, Report | 入金・出金（journal_text含む） |

**ルーティングロジック**: Cloud Sync Service は受信した TransactionLog の `log_type` フィールドに基づき、上記対応表に従って適切なトピックに配信します。Journal Service はすべてのトピックをサブスクライブし、`journal_text` フィールドを抽出して電子ジャーナルとして保存します。
...
| トランザクションログ収集 | cart, terminal → Sync | **Pub/Sub (Push)** | トピック: `tranlog_report`, `cashlog_report`, `opencloselog_report` | 既存トピック活用、リアルタイム性（journal_textフィールドを含む） |
...
- **対象データ**: JSON形式のマスターデータ、トランザクションログ（journal_textフィールド含む）
...
**Edge → Cloud (Push方式)**:
- `POST /api/v1/sync/transaction-logs` - トランザクションログのpush（journal_textフィールドを含む）
...
| **Push (Pub/Sub)** | トランザクションログ（journal_text含む） | 既存トピック活用、複数サービス（cart, terminal, journal）への配信、リアルタイム性 |
...
- **サービス間通信**: ✅ Dapr pub/sub（トランザクションログ受信、journal_text経由でJournalサービスへ配信）、Service Invocation（Master-dataサービス、Terminalサービスとの通信）
...
- **contracts/*.yaml**: ✅ Dapr Service Invocation使用を明示（master-data、terminalサービスとの通信）、Dapr Pub/Sub使用を明示（journal_text経由でJournalサービスへ配信）
...
1. **Dapr Service Invocation の実装パターン**: Master-dataサービス、Terminalサービスとの通信方法
