# ユニットテスト: get_snapshot_by_id
# API: GET /tenants/{tenant_id}/stores/{store_code}/stock/snapshot/{snapshot_id}
# 実装日: 2026-03-10
...
@pytest.mark.asyncio
async def test_get_snapshot_by_id_unit_success(async_client):
    """
    【正常系 | P0 ユニットテスト】
    ...
    test_snapshot_id = "000000000000000000000001"
    try:
        response = await async_client.get(
            f"/api/v1/tenants/{test_test_tenant_id}/stores/{test_test_store_code}/stock/snapshot/{test_snapshot_id}",
        )
...
@pytest.mark.asyncio
async def test_get_snapshot_by_id_unit_error_handling(async_client):
    """
    【異常系 | P0 ユニットテスト】
