# ユニットテスト: get_low_stocks
# API: GET /tenants/{tenant_id}/stores/{store_code}/stock/low
# 実装日: 2026-03-10
...
@pytest.mark.asyncio
async def test_get_low_stocks_unit_success(async_client):
    """
    【正常系 | P0 ユニットテスト】
    GET /tenants/{tenant_id}/stores/{store_code}/stock/low
    ...
    try:
        response = await async_client.get(
            f"/api/v1/tenants/{test_test_tenant_id}/stores/{test_test_store_code}/stock/low",
        )
...
@pytest.mark.asyncio
async def test_get_low_stocks_unit_error_handling(async_client):
    """
    【異常系 | P0 ユニットテスト】
