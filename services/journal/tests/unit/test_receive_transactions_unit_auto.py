# ユニットテスト: receive_transactions
# API: POST /tenants/{tenant_id}/stores/{store_code}/terminals/{terminal_no}/transactions
# 実装日: 2026-03-10
...
@pytest.mark.asyncio
async def test_receive_transactions_unit_success(async_client):
    """
    【正常系 | P0 ユニットテスト】
    ...
    try:
        response = await async_client.post(
            f"/api/v1/tenants/{test_test_tenant_id}/stores/{test_test_store_code}/terminals/{test_terminal_no}/transactions",
        json={},
        )
...
@pytest.mark.asyncio
async def test_receive_transactions_unit_error_handling(async_client):
    """
    【異常系 | P0 ユニットテスト】
