# ユニットテスト: update_terminal_function_mode
# API: PATCH /terminals/{terminal_id}/function_mode
# 実装日: 2026-03-10
...
@pytest.mark.asyncio
async def test_update_terminal_function_mode_unit_success(async_client):
    """
    【正常系 | P0 ユニットテスト】
    ...
    try:
        response = await async_client.patch(
            f"/api/v1/terminals/{test_terminal_id}/function_mode",
        json={},
        )
...
@pytest.mark.asyncio
async def test_update_terminal_function_mode_unit_error_handling(async_client):
    """
    【異常系 | P0 ユニットテスト】
    ...
    pytest.skip(
        ...
        f" 対象: update_terminal_function_mode"
    )
