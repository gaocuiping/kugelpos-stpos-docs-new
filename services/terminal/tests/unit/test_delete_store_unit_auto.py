# ユニットテスト: delete_store
# API: DELETE /tenants/{tenant_id}/stores/{store_code}
# 実装日: 2026-03-10
...
@pytest.mark.asyncio
async def test_delete_store_unit_success(async_client):
    """
    【正常系 | P0 ユニットテスト】
    DELETE /tenants/{tenant_id}/stores/{store_code}
    ...
    try:
        response = await async_client.delete(
            f"/api/v1/tenants/{test_test_tenant_id}/stores/{test_test_store_code}",
        )
...
@pytest.mark.asyncio
async def test_delete_store_unit_error_handling(async_client):
    """
    【異常系 | P0 ユニットテスト】
