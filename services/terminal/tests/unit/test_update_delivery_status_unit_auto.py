# ユニットテスト: update_delivery_status
# API: POST /terminals/{terminal_id}/delivery-status
# 実装日: 2026-03-10
...
@pytest.mark.asyncio
async def test_update_delivery_status_unit_success(async_client):
    """
    【正常系 | P0 ユニットテスト】
    ...
    try:
        response = await async_client.post(
            f"/api/v1/terminals/{test_terminal_id}/delivery-status",
        json={},
        )
...
@pytest.mark.asyncio
async def test_update_delivery_status_unit_error_handling(async_client):
    """
    【異常系 | P0 ユニットテスト】
