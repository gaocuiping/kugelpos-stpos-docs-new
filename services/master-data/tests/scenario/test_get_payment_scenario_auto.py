# テストケース: GET /tenants/{tenant_id}/payments/{payment_code}
# API: get_payment
# 実装日: 2026-03-10
...
@pytest.mark.asyncio
async def test_get_payment_happy_path(
    ...
    """
    【正常系 | P1-4】
    GET /tenants/{tenant_id}/payments/{payment_code}
    - 正常なリクエストに対して 200 が返ること
    - レスポンスが success=True であること
    """
    test_payment_code = "TEST_PAYMENT_CODE"  # TODO: 有効な値に差し替えてください
...
async def test_get_payment_not_found(
    ...
    response = await http_client.get(
        f"/api/v1/tenants/{test_test_tenant_id}/payments/NONEXISTENT_000",
        headers=test_auth_headers,
    )
