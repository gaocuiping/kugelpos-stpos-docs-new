# テストケース: GET /tenants/{tenant_id}/items/{item_code}
# API: get_item_master_async
# 実装日: 2026-03-10
...
@pytest.mark.asyncio
async def test_get_item_master_async_happy_path(
...
@pytest.mark.asyncio
async def test_get_item_master_async_not_found(
    http_client: AsyncClient,
    test_tenant_id: str,
    test_store_code: str,
    test_auth_headers: dict,
    ...
    response = await http_client.get(
        f"/api/v1/tenants/{test_test_tenant_id}/items/NONEXISTENT_000",
        headers=test_auth_headers,
    )
...
@pytest.mark.asyncio
async def test_get_item_master_async_unauthorized(
    http_client: AsyncClient,
    test_tenant_id: str,
    test_store_code: str,
):
