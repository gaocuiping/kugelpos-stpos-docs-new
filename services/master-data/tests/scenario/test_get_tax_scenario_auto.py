# テストケース: GET /tenants/{tenant_id}/taxes/{tax_code}
# API: get_tax
# 実装日: 2026-03-10
...
@pytest.mark.asyncio
async def test_get_tax_happy_path(
    ...
    """
    【正常系 | P1-4】
    GET /tenants/{tenant_id}/taxes/{tax_code}
    - 正常なリクエストに対して 200 が返ること
    - レスポンスが success=True であること
    """
    test_tax_code = "TEST_TAX_CODE"  # TODO: 有効な値に差し替えてください
    response = await http_client.get(
        f"/api/v1/tenants/{test_test_tenant_id}/taxes/{test_tax_code}",
...
async def test_get_tax_not_found(
    ...
    response = await http_client.get(
        f"/api/v1/tenants/{test_test_tenant_id}/taxes/NONEXISTENT_000",
        headers=test_auth_headers,
    )
...
@pytest.mark.asyncio
async def test_get_tax_unauthorized(
    http_client: AsyncClient,
    test_tenant_id: str,
    test_store_code: str,
):
