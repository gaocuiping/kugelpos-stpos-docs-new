# ユニットテスト: get_item_master_all_async
# API: GET /tenants/{tenant_id}/items
# 実装日: 2026-03-10
...
@pytest.mark.asyncio
async def test_get_item_master_all_async_unit_success(async_client):
    """
    【正常系 | P0 ユニットテスト】
    ...
    try:
        response = await async_client.get(
            f"/api/v1/tenants/{test_test_tenant_id}/items",
        )
...
@pytest.mark.asyncio
async def test_get_item_master_all_async_unit_error_handling(async_client):
    """
    【異常系 | P0 ユニットテスト】
    ...
    pytest.skip(
        ...
        f" 対象: get_item_master_all_async"
    )
