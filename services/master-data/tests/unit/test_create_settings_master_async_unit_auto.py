# ユニットテスト: create_settings_master_async
# API: POST /tenants/{tenant_id}/settings
# 実装日: 2026-03-10
...
@pytest.mark.asyncio
async def test_create_settings_master_async_unit_success(async_client):
    """
    【正常系 | P0 ユニットテスト】
    ...
    try:
        response = await async_client.post(
            f"/api/v1/tenants/{test_test_tenant_id}/settings",
        json={},
        )
...
@pytest.mark.asyncio
async def test_create_settings_master_async_unit_error_handling(async_client):
    """
    【異常系 | P0 ユニットテスト】
    ...
    pytest.skip(
        ...
        f" 対象: create_settings_master_async"
    )
