# ユニットテスト: delete_category
# API: DELETE /tenants/{tenant_id}/categories/{category_code}
# 実装日: 2026-03-10
...
@pytest.mark.asyncio
async def test_delete_category_unit_success(async_client):
    """
    【正常系 | P0 ユニットテスト】
    DELETE /tenants/{tenant_id}/categories/{category_code}
    ...
    try:
        response = await async_client.delete(
            f"/api/v1/tenants/{test_test_tenant_id}/categories/{test_category_code}",
        )
...
@pytest.mark.asyncio
async def test_delete_category_unit_error_handling(async_client):
    """
    【異常系 | P0 ユニットテスト】
