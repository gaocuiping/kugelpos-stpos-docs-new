...
async def test_register_invoice_number(set_env_vars):
    ...
    # 登録結果の確認 (既に存在する場合は400を許容)
    if result.get("code") == 400 and "already exists" in result.get("message", ""):
        print("INVOICE_REGISTRATION_NUMBER already exists - skipping registration")
    else:
    ...
    if result.get("code") == 400 and "already exists" in result.get("message", ""):
        print("RECEIPT_HEADERS already exists - skipping registration")
    else:
        assert result.get("success") is True
        assert result.get("code") == status.HTTP_201_CREATED
    ...
    if result.get("code") == 400 and "already exists" in result.get("message", ""):
        print("RECEIPT_FOOTERS already exists - skipping registration")
    else:
        assert result.get("success") is True
        assert result.get("code") == status.HTTP_201_CREATED
